...
def main():
    ...
    ax.scatter(x, ratings, color=BLUE, s=40, zorder=4, edgecolors=BG, linewidths=1.2)

    peak_i = ratings.index(max(ratings))
    ax.scatter([x[peak_i]], [ratings[peak_i]], color=MAUVE, s=140, zorder=5, marker="*", edgecolors=BG, linewidths=0.8)

    if peak_i == len(ratings) - 1:
        ax.annotate(f"peak & current - {ratings[peak_i]}", (x[peak_i], ratings[peak_i]),
                    textcoords="offset points", xytext=(-70, 14), ha="center", fontsize=9, color=MAUVE)
    else:
        ax.annotate(f"peak {ratings[peak_i]}", (x[peak_i], ratings[peak_i]),
                    textcoords="offset points", xytext=(0, 12), ha="center", fontsize=9, color=MAUVE)
    ...
    ax.text(1.0, 1.12, f"{len(history)} rated contests", transform=ax.transAxes,
            ha="right", fontsize=8.5, color=SUBTEXT, style="italic")

...
def main():
    ...
    ax.scatter(x, ranks, color=TEAL, s=55, zorder=4, edgecolors=BG, linewidths=1.5)

    best_i = ranks.index(min(ranks))
    ax.scatter([x[best_i]], [ranks[best_i]], color=MAUVE, s=160, zorder=5, marker="*", edgecolors=BG, linewidths=0.8)
    ax.annotate(f"personal best - rank {ranks[best_i]}", (x[best_i], ranks[best_i]),
                textcoords="offset points", xytext=(-55, 14), ha="center", fontsize=10, color=MAUVE)

    for xi, yi in zip(x, ranks):
        if xi == best_i:
            continue
        ax.annotate(f"{yi}", (xi, yi), textcoords="offset points", xytext=(0, 10),
                    ha="center", fontsize=9.5, color=SUBTEXT)

    ax.set_xticks(x)
    ax.set_xticklabels(labels, color=SUBTEXT, fontsize=9.5)
    ax.invert_yaxis()
    ax.set_ylabel("rank (lower is better)", color=SUBTEXT, fontsize=10)
    ax.tick_params(axis="y", colors=SUBTEXT, labelsize=9.5)
    ...
    if pending_count:
        ax.text(1.0, 1.14, f"+{pending_count} more onsite contests scheduled - awaiting results",
                transform=ax.transAxes, ha="right", fontsize=9.5, color=LAVENDER, style="italic")
